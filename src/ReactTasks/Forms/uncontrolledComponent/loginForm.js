...
export default function LoginForm() {
  ...
  let FormSubmit = (e) => {
    ...
    var localStorageData = JSON.parse(localStorage.getItem("loginUsers"));
    if (localStorageData == null) {
      let loginUsers = [];
      loginUsers.push(obj);
      localStorage.setItem("loginUsers", JSON.stringify(loginUsers));
    } else {
      localStorageData.map((eachData) => {
        if (eachData.email == obj.email) {
          userCheck++;
        }
      });
    }
    if (userCheck === 0) {
      localStorage.setItem(
        "loginUsers",
        JSON.stringify(...localStorageData, obj)
      );
    }
  };
  return (
